...
editHistory:
  ...
  - date: 2026-05-05
    changes: Corrected course for the private maintainer dashboard so the planning baseline now treats it as a readiness-first trust-and-readiness console with stronger triage, audit, validation, and maintainability expectations.
briefCount: 1
researchCount: 0
...
The private admin side should be treated as a maintainer-only trust-and-readiness console. Its MVP role is to protect the grounded chatbot model by supporting approved-source stewardship, ingestion readiness, chunk and citation inspection, validation runs, audit history, and demo-readiness checks. It should answer "Is the system ready for demo, what is blocked, and what should the maintainer do next?" quickly through a readiness-first overview, dedicated operational sections, and source-linked follow-up paths. It is not a public CMS, learner dashboard, analytics portal, LMS, or general AI control panel.

MVP admin capabilities may include private maintainer login, protected route access, a blocker-driven readiness overview, dedicated dashboard, sources, ingestion, validation, and audit views, source list and source detail triage views, source upload as draft, metadata editing, enable or disable or archive actions, ingestion trigger and status visibility, chunk and citation inspection, validation runner and validation history, source-linked audit logs, protected Django APIs, runtime validation, user-safe errors, and tests for protected admin behavior.
