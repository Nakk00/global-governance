-- Repair databases that applied an earlier persistence function without vectors.
...
declare
  ...
  chunk_id text;
  expected_chunk_count integer := jsonb_array_length(payload -> 'chunks');
  expected_embedding_dimensions integer :=
    (document_payload -> 'embeddingConfig' ->> 'dimensions')::integer;
  persisted_embedding_count integer;
  persisted_embedding_dimensions integer;
begin
  ...
  loop
    ...
    values (
      ...
      (chunk_payload -> 'embedding')::text::extensions.vector,
  ...
  select
    count(embedding),
    min(extensions.vector_dims(embedding))
  into
  ...
  if persisted_embedding_count != expected_chunk_count
    or persisted_embedding_dimensions is distinct from expected_embedding_dimensions
  then
    raise exception 'Persisted embedding evidence is incomplete';
  end if;
  ...
  return jsonb_build_object(
    ...
    'embeddingCount', persisted_embedding_count,
    'embeddingDimensions', persisted_embedding_dimensions
  );
end;
$$;
